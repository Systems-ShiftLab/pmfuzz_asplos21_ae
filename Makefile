...
# Enable ngram: 
# 	"Be Sensitive and Collaborative: Analzying Impact of Coverage Metrics in Greybox Fuzzing", by Jinghan Wang, et. al.
export AFL_LLVM_NGRAM_SIZE = 4

# Enable context sensitive coverage: 
# 	https://github.com/AFLplusplus/AFLplusplus/blob/stable/llvm_mode/README.ctx.md
...
BUGGY_REDIS_DIR			 = $(DIR)vendor/redis-3.2-nvml-buggy/
...
export REDIS_CFLAGS		 = -I$(INCLUDE_DIR) $(PMFUZZ_CFLAGS) $(BUG_SWITCH)
...
BUGGY_MEMCACHED_DIR		:= $(DIR)vendor/memcached-pmem-buggy/
...
BUGGY_PMDK_DIR			:= $(DIR)vendor/pmdk-buggy/
...
export EXTRA_CFLAGS 	+= $(BUG_SWITCH)
...
buggy-redis: $(BUILD_DIR).buggy_redis_deps
	...
	(cd $(BUGGY_REDIS_DIR) && $(MAKE) USE_PMDK=yes STD=-std=gnu99 \
		...
		$(EXTRA_LDFLAGS)")
	$(MAKE) $(BIN_DIR)buggy_redis-server 
...
$(BUILD_DIR).buggy_memcached_configured:
	$(eval EXTRA_LIBS := $(filter-out -lpmfuzz,$(EXTRA_LIBS)))
	$(eval EXTRA_LIBS := $(EXTRA_LIBS) -lfakepmfuzz)
	@printf '  %b %b\n' $(MAKECOLOR)MAKE$(ENDCOLOR) $(BINCOLOR)$@$(ENDCOLOR)
	(cd $(BUGGY_MEMCACHED_DIR) && CFLAGS="$(EXTRA_CFLAGS)" LDFLAGS="$(EXTRA_LDFLAGS)"\
		LIBS="$(EXTRA_LIBS)" ./configure --enable-pslab \
		--disable-coverage)
	@touch $@

$(BIN_DIR)buggy_memcached:
	$(QUIET_LN)ln -Lfs $(BUGGY_MEMCACHED_DIR)memcached $@

#BRIEF: Builds buggy Memcached
buggy-memcached: $(BUILD_DIR).buggy_memcached_configured
	$(eval EXTRA_LIBS := $(filter-out -lpmfuzz,$(EXTRA_LIBS)))
	$(eval EXTRA_LIBS := $(EXTRA_LIBS) -lfakepmfuzz)
	(cd $(BUGGY_MEMCACHED_DIR) && CFLAGS="$(EXTRA_CFLAGS)" LDFLAGS="$(EXTRA_LDFLAGS)"\
		LIBS="$(EXTRA_LIBS)" ./configure --enable-pslab --disable-coverage)
	$(MAKE) -C $(BUGGY_MEMCACHED_DIR)
	$(MAKE) $(BIN_DIR)buggy_memcached

#BRIEF: Builds mariadb
mariadb:
	(cd $(MARIA_DB_DIR) && BUILD/compile-custom)
...
$(LLVM_DIR):
	...
	wget -P $(BUILD_DIR) -c 'https://releases.llvm.org/9.0.0/clang+llvm-9.0.0-x86_64-linux-gnu-ubuntu-18.04.tar.xz'
	mkdir -p $(LLVM_DIR)
...
#BRIEF: Builds PMDK with bugs
buggy-pmdk:
	$(eval EXTRA_LIBS := $(filter-out -lpmfuzz,$(EXTRA_LIBS)))
	$(eval EXTRA_LIBS := $(EXTRA_LIBS) -lfakepmfuzz)
	@printf '  %b %b\n' $(MAKECOLOR)MAKE$(ENDCOLOR) $(BINCOLOR)$@$(ENDCOLOR)
	$(MAKE) -C $(BUGGY_PMDK_DIR)
	sudo -E $(MAKE) -C $(BUGGY_PMDK_DIR) install
...
$(DOCS_DIR)programming_manual.pdf:
	$(QUIET_LN)ln -fs $(PMFUZZ_DIR)docs/latex/refman.pdf  $(DOCS_DIR)programming_manual.pdf
