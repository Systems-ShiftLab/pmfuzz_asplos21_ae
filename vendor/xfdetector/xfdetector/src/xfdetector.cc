...
{
    // cerr << "@ " << __LINE__ <<  " exec_id = " << exec_id << endl;
    // Initialize FIFOs
    fifo_create(exec_id);
    /* Move to main function, after execute pintool */
        ...
        switch (cur_trace->operation) {
            ...
            case CLWB:
                if (stage == PRE_FAILURE) {
                    ...
                    // cerr << "@XFD CLWB: " << std::hex << dst_addr << " " << size << endl;
                }
                break;
            case SFENCE:
                shadow_mem->drain_writeback(cur_trace);
            ...
            case PM_TRACE_TX_ADDR_ADD:
                // cerr << "@XFD PM_TRACE_TX_ADDR_ADD: " << std::hex << dst_addr << " " << size << endl;
                // if(stage == POST_FAILURE){
                    // shadow_mem->set_consistent_addr(cur_trace, dst_addr, size);
                // }
                // if (stage == PRE_FAILURE) {
...
int main(int argc, char* argv[])
{
    ...
    while (race_detector.pre_testing_complete != COMPLETE) {
        ...
        while (race_detector.post_testing_complete != COMPLETE) {
            ...
            if (POST_FAILURE_EXEC_TIMEOUT > 0 && post_end.tv_sec - post_start.tv_sec > POST_FAILURE_EXEC_TIMEOUT) {
