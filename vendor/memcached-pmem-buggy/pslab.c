...
void pslab_use_slab(void *p, int id, unsigned int size) {
    ...
    fp->id = id;
    #ifdef MEMCACHED_PSLAB_3
    dprintf(3, "PMFuzz found: pslab_3\n");
    #else
...
static void pslab_checksum_init() {
    ...
    #ifdef MEMCACHED_PSLAB_perf_2
    pmem_persist(pslab_chksum0, sizeof(pslab_chksum0));
    dprintf(3, "PMFuzz found: pslab_perf_2\n");
    #endif
...
void pslab_update_flushtime(uint32_t time) {
    ...
    #ifdef MEMCACHED_PSLAB_8
    dprintf(3, "PMFuzz found: pslab_8\n");
    #else
...
int pslab_do_recover() {
    ...
    #ifdef MEMCACHED_PSLAB_PERF_3
    pmem_persist(settings.oldest_live, sizeof(settings.oldest_live));
    dprintf(3, "PMFuzz found: pslab_perf_3\n");
    #endif
    ...
    PSLAB_WALK(fp) {
        ...
        for (i = 0, ptr = fp->slab; i < perslab; i++, ptr += size) {
            ...
            if (it->it_flags & ITEM_LINKED) {
                if (item_is_flushed(it) ||
                    ...
                    #ifdef MEMCACHED_PSLAB_1
                    dprintf(3, "PMFuzz found: pslab_1\n");
                    #else
                    ...
                    #ifdef MEMCACHED_PSLAB_PERF_1
                    pmem_persist(fp->flags, sizeof(fp->flags));
                    dprintf(3, "PMFuzz found: pslab_perf_1\n");
                    #endif
                }
        ...
        #ifdef MEMCACHED_PSLAB_9
        pmem_persist(ptr, sizeof(fp->slab));
        dprintf(3, "PMFuzz found: pslab_9\n");
        #endif
    }
    ...
    PSLAB_WALK(fp) {
        ...
        #ifdef MEMCACHED_PSLAB_10
        pmem_persist(ptr, sizeof(fp->slab));
        dprintf(3, "PMFuzz found: pslab_10\n");
        #endif
    }
    ...
    PSLAB_WALK(fp) {
        ...
        #ifdef MEMCACHED_PSLAB_11
        pmem_persist(ptr, sizeof(fp->slab));
        dprintf(3, "PMFuzz found: pslab_11\n");
        #endif
    }
...
int pslab_create(char *pool_name, uint32_t pool_size, uint32_t slab_page_size,
    ...
    #ifdef MEMCACHED_PSLAB_6
    pmem_memset(pslab_pool, 0, length);
    dprintf(3, "PMFuzz found: pslab_6\n");
    #else
    ...
    PSLAB_WALK(fp) {
        #ifdef MEMCACHED_PSLAB_7
        pmem_memset(fp, 0, sizeof (pslab_t));
        dprintf(3, "PMFuzz found: pslab_7\n");
        #else
    ...
    #ifdef MEMCACHED_PSLAB_5
    dprintf(3, "PMFuzz found: pslab_5\n");
    #else
    pmem_member_persist(pslab_pool, valid);
    #endif
