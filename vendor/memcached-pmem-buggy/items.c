...
item *do_item_alloc(char *key, const size_t nkey, const unsigned int flags,
    ...
    if (it->it_flags & ITEM_CHUNKED) {
        ...
        if (it->it_flags & ITEM_PSLAB) {
            ...
            // dprintf(3, "PMFuzz found: items_6\n");
        }
#endif
    }
    it->h_next = 0;
...
int do_item_link(item *it, const uint32_t hv) {
    ...
    if (it->it_flags & ITEM_PSLAB) {
        ...
        // items_5 place the following persist to the end of the if-statement
...
#ifdef PSLAB
        if (it->it_flags & ITEM_PSLAB) {
            #ifdef MEMCACHED_ITEMS_2
            dprintf(3, "PMFuzz found: items_2\n");
            #else
            pmem_member_persist(it, it_flags);
            #endif
        }
...
void do_item_update_nolock(item *it) {
    ...
    if (it->time < current_time - ITEM_UPDATE_INTERVAL) {
        // dprintf(3, "PMFuzz found: items_5\n");
        assert((it->it_flags & ITEM_SLABBED) == 0);
