...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    // PMFuzz: Hacky stuff for image fuzzing
    if (getenv("PMFUZZ_MEMCACHED_EXIT_ON_COMMAND_READ") != NULL) {
        printf("PMFuzz early exit env variable (PMFUZZ_MEMCACHED_EXIT_ON_COMMAND_READ) detected, exiting...\n");
        exit(0);
    }
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv, shortopts))) {
        ...
        case 'X' :
            ...
            break;
        case 'j' :
            // PMFuzz: Custom option to allow <comma> in file path, getsubopt doesn't allow that
            pslab_file = strdup(optarg);

            // PMFuzz: In no forksrv mode remove the existing file so that redis
            // creates a new pool
            int file_found = access(pslab_file, F_OK);
            ...
            // PMFuzz if the file exists, set pslab_recover to true
            struct stat buffer;
            if (stat (pslab_file, &buffer) == 0) {
                printf("memcached: pslab file found, will attempt recovery\n");
...
#ifdef PSLAB
            case PSLAB_FILE:
                printf("Syntax modified for PMFuzz, check help.\n");
                return 1;
