...
int pslab_create(char *pool_name, uint32_t pool_size, uint32_t slab_page_size,
    ...
    printf("Creating a pmem pool at %s\n", pool_name);
    ...
    pmem_memset_nodrain_pmfuzz(pslab_pool, 0, length);
    ...
    PSLAB_WALK(fp) {
        pmem_memset_nodrain_pmfuzz(fp, 0, sizeof (pslab_t));
    }
    ...
    pmem_persist_pmfuzz(pslab_pool, pslab_pool->length);
